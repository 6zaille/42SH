...
static void save_fd(int fd) {
    struct saved_fd *new_saved_fds = realloc(saved_fds, (saved_fd_count + 1) * sizeof(struct saved_fd));
    if (!new_saved_fds) {
        perror("realloc");
        exit(EXIT_FAILURE);
    }
...
void execute_with_redirections(struct ast *node) {
    ...
    for (size_t i = 0; i < data->redirection_count; i++) {
        ...
        switch (redir->type) {
            case REDIR_OUT:
                apply_redirection(redir->filename, STDOUT_FILENO, O_WRONLY | O_CREAT | O_TRUNC, 0644);
                break;
            case REDIR_IN:
                apply_redirection(redir->filename, STDIN_FILENO, O_RDONLY, 0);
                break;
            case REDIR_APPEND:
                apply_redirection(redir->filename, STDOUT_FILENO, O_WRONLY | O_CREAT | O_APPEND, 0644);
                break;
            case REDIR_DUP_OUT:
            ...
            case REDIR_DUP_IN:
                ...
                break;
            case REDIR_CLOBBER:
                apply_redirection(redir->filename, STDOUT_FILENO, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
                break;
            case REDIR_RW:
                apply_redirection(redir->filename, STDIN_FILENO, O_RDWR | O_CREAT, 0644);
                break;
            default:
